...
from AbhiXMusic import app, Userbot
...
from AbhiXMusic.utils.admin_filters import admin_filter
from config import BANNED_USERS

@app.on_message(filters.command(["vcinfo"], ["/", "!"]))
async def strcall(client, message):
    assistant = await group_assistant(Abhi, message.chat.id)
    try:
        await assistant.join_group_call(message.chat.id, AudioPiped("./AbhiXMusic/assets/call.mp3"), stream_type=StreamType().pulse_stream)
...
async def set_volume(client, message: Message):
    ...
    try:
        await Abhi.change_volume(chat_id, volume_level)
