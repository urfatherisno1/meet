# Owner @UR_Father
from AbhiXMusic import app as app
...
async def whispes_cb(_, query):
    ...
    if user_id not in [from_user, to_user, 8302512047]:
        try:
...
async def in_help():
    answers = [
        InlineQueryResultArticle(
            title="💒 Whisper",
            description=f"@RockXMusic_Robot [USERNAME | ID] [TEXT]",
            input_message_content=InputTextMessageContent(f"**📍Usage:**\n\n@RockXMusic_Robot (Target Username or ID) (Your Message).\n\n**Example:**\n@RockXMusic_Robot @username I Wanna Phuck You"),
            thumb_url="https://te.legra.ph/file/3eec679156a393c6a1053.jpg",
            reply_markup=switch_btn
        )
    ]
